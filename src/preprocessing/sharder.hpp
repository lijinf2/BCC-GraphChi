...
#define SHARDER_BUFSIZE (64 * 1024 * 1024)
    ...
    struct shovel_merge_source : public merge_source<edge_with_value<EdgeDataType> > {
        ...
        size_t idx;    // counting added edge in one file, which can be used in reading data with offset(saving memory)
        size_t bufidx;  // buf idx // used for load part of the shovel file one part.
        edge_with_value<EdgeDataType> * buffer;
        int f;
        size_t numedges;
        ...
        void load_next() {
            size_t len = std::min(bufsize_bytes,  ((numedges - idx) * sizeof(edge_with_value<EdgeDataType>))); // in order to read the final remaining data
    ...
    public:
        ...
        void start_preprocessing() {
            ...
            numshovels = 0;
            //shovelsize store number of edge_with_value in a shovel
        ...
        void flush_shovel(bool async=true) {
            ...
            if (!async) {  // this round is for the last shovel, which may not be full
                curshovel_buffer = NULL;
                flushinfo->flush();
        ...
        void bwrite_edata(char * &buf, char * &bufptr, T val, size_t & totbytes, std::string & shard_filename,
            ...
            if (edgecounter == edges_per_block) { // enough then write into file:w

                edata_flush<T>(buf, bufptr, shard_filename, totbytes);
               // std::cout<<" atttention, edata_flush"<<std::endl;
               // char chh;
               // std::cin>>chh;
                bufptr = buf;
                edgecounter = 0;
    ...
    protected:
        ...
        virtual void finish_shard(int shard, edge_t * shovelbuf, size_t shovelsize) {
...
#else
            quickSort(shovelbuf, (int)numedges, edge_t_src_less<EdgeDataType>); // sort by src
#endif
            m.stop_time("finish_shard.sort");

            // Remove duplicates
            if (duplicate_edge_filter != NULL && numedges > 0) { // no used normally
            ...
            for(size_t i=0; i <= numedges; i++) { // targe every edge
                ...
                if (!edge.stopper()) {
                    ...
                    //write data into ebuf， then to edfname file if enough big
#else
                ...
                if ((edge.src != curvid) || edge.stopper()) {
                    ...
                    // write dsts of the new vertex after write count:w
        ...
        virtual void add(edge_with_value<EdgeDataType> val) {
            ...
            if (cur_shard_counter == shard_capacity) { // It's not needed currently
        ...
        virtual void write_shards() {
           ...
           // std::cout<<"after push_back shovel"<<std::endl;
            //char x;//
           // std::cin>>x;//

            kway_merge<edge_with_value<EdgeDataType> > merger(sources, this);
           //std::cout<<"after kway_merge"<<std::endl;
           // std::cin>>x;
            merger.merge();
           // std::cout<<"after merge, files system appears"<<std::endl;
           // std::cin>>x;//
            // Delete sources
