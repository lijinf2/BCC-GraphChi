...
namespace graphchi {
    ...
    void convert_edgelist(std::string inputfile, sharder<EdgeDataType, FinalEdgeDataType> &sharderobj, bool multivalue_edges=false) {
        ...
        while(fgets(s, 1024, inf) != NULL) {
            ...
            if (s[0] == '%') continue; // Comment
            if (s[0] == 'c') continue; // comment
            if (s[0] == 'p') continue; // comment
            ...
            //std::cout<<"from value of t is: "<<t<<"\n";
            if(strcmp(t,"a") == 0) t = strtok(NULL,delims);// in the graph, the input file may be different. This may be a 'a', we ignore this character
            //std::cout<<"another from value of t is: "<<t<<"\n";
            if (t == NULL) {
                logstream(LOG_ERROR) << "Input file is not in right format. "
                << "Expecting \"<from>\t<to>\". "
                << "Current line: \"" << s << "\"\n";
